...
class Przyslowie(BaseCommand):

    def __init__(self):
        description = "Przysłowia na dziś"
        params = []
    ...
    async def handle(self, params, message, client):
        ...
        if len(params)>0:
            dz=False
            p1 = message.content[len(settings.COMMAND_PREFIX)+10:].strip()
            try:
        ...
        msg=[]
        for j in c.execute("SELECT przyslowie FROM calendar WHERE day=? AND month =? LIMIT 1;", (d,m1) ):
        ...
        embed.title = "Przysłowia na dziś" if dz else "Przysłowia z {}".format(dzis)
        embed.description = msg
        #await message.channel.send(embed=embed)
        await utils.send(client= client, message=message, cmd="przyslowie",embed=embed)
