...
async def handle_command(command, args, message, bot_client):
    ...
    # with some other bot)
    command = command.replace("ż","z").replace("ę","e").replace("ł","l").replace("ń","n").replace("ź","z").replace("ó","o").replace("ą","a").replace("ę","e").replace("ć","c")
    ...
    if not bot_client.user.id == message.author.id: 
        await utils.stat(cmd=command, srv=message.guild.id)